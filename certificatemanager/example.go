...
func example() {
	...
	// Step 2: declare pointer for model string
	var cModelStr *C.char

	// Step 3: call zkGetModelNumberString
	result := C.zkGetModelNumberString(zkptr, &cModelStr)
	if result < 0 {
		fmt.Println("zkGetModelNumberString failed")
		return
	}

	// Step 4: convert C string to Go string
	modelStr := C.GoString(cModelStr)
	fmt.Println("Zymkey Model:", modelStr)

	// Step 5: free the allocated C string
	C.free(unsafe.Pointer(cModelStr))
	//*
	//* close and exit
	//*
