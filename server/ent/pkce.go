...
// PKCE is the model entity for the PKCE schema.
type PKCE struct {
	config `json:"-"`
	// ID of the ent.
...
func (*PKCE) scanValues(columns []string) ([]any, error) {
...
// to the PKCE fields.
func (pk *PKCE) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case pkce.FieldCode:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				pk.Code = value.String
			}
		case pkce.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field pkce_session", value)
			} else if value.Valid {
...
func (pk *PKCE) Unwrap() *PKCE {
	_tx, ok := pk.config.driver.(*txDriver)
	if !ok {
		panic("ent: PKCE is not a transactional entity")
	}
...
func (pk *PKCE) String() string {
	var builder strings.Builder
	builder.WriteString("PKCE(")
	builder.WriteString(fmt.Sprintf("id=%v, ", pk.ID))
	builder.WriteString("code=")
	builder.WriteString(pk.Code)
	builder.WriteByte(')')
