...
type AuthCodeQuery struct {
	...
	loadTotal   []func(context.Context, []*AuthCode) error
...
func (acq *AuthCodeQuery) QuerySession() *OAuthSessionQuery {
	query := &OAuthSessionQuery{config: acq.config}
...
func (acq *AuthCodeQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (acq *AuthCodeQuery) Clone() *AuthCodeQuery {
	...
	return &AuthCodeQuery{
		...
		withSession: acq.withSession.Clone(),
...
func (acq *AuthCodeQuery) WithSession(opts ...func(*OAuthSessionQuery)) *AuthCodeQuery {
	query := &OAuthSessionQuery{config: acq.config}
	for _, opt := range opts {
		opt(query)
	}
	acq.withSession = query
	return acq
}
...
func (acq *AuthCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthCode, error) {
	...
	if query := acq.withSession; query != nil {
		if err := acq.loadSession(ctx, query, nodes, nil,
			func(n *AuthCode, e *OAuthSession) { n.Edges.Session = e }); err != nil {
			return nil, err
		}
	}
...
func (acq *AuthCodeQuery) loadSession(ctx context.Context, query *OAuthSessionQuery, nodes []*AuthCode, init func(*AuthCode), assign func(*AuthCode, *OAuthSession)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuthCode)
	for i := range nodes {
		if nodes[i].auth_code_session == nil {
			continue
		}
		fk := *nodes[i].auth_code_session
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "auth_code_session" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
