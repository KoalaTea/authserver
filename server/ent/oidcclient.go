...
func (*OIDCClient) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case oidcclient.FieldRedirectUris, oidcclient.FieldResponseTypes, oidcclient.FieldGrantTypes, oidcclient.FieldScopes:
			values[i] = new([]byte)
		case oidcclient.FieldID:
			values[i] = new(sql.NullInt64)
		case oidcclient.FieldClientID, oidcclient.FieldSecret:
			values[i] = new(sql.NullString)
		default:
			return nil, fmt.Errorf("unexpected column %q for type OIDCClient", columns[i])
		}
	}
	return values, nil
}
...
func (oc *OIDCClient) Update() *OIDCClientUpdateOne {
	return (&OIDCClientClient{config: oc.config}).UpdateOne(oc)
}
...
func (oc OIDCClients) config(cfg config) {
	for _i := range oc {
		oc[_i].config = cfg
	}
}
